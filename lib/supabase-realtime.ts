...
function nextChannelName(base: string) {
  channelSequence += 1;
  return `${base}:${channelSequence}`;
}
...
export function subscribeToDealMessages(dealId: string, onChange: (payload: RealtimeMessagePayload) => void) {
  ...
  const channel: RealtimeChannel = client.channel(nextChannelName(`deal-messages:${dealId}`))
...
export function subscribeToMessages(onChange: (payload: RealtimeMessagePayload) => void) {
  const client = getSupabaseRealtimeClient();
  if (!client) return null;
  const channel: RealtimeChannel = client.channel(nextChannelName("messages-global"))
...
export function subscribeToMessengerRoom(roomId: string, onChange: (payload: RealtimeMessagePayload) => void) {
  const client = getSupabaseRealtimeClient();
  if (!client) return null;
  const channel: RealtimeChannel = client.channel(nextChannelName(`messenger-room:${roomId}`))
...
export function subscribeToMessengerMessages(onChange: (payload: RealtimeMessagePayload) => void) {
  const client = getSupabaseRealtimeClient();
  if (!client) return null;
  const channel: RealtimeChannel = client.channel(nextChannelName("messenger-messages-global"))
