...
export type AdminAuditLog = { id: string; action: "set_verified" | "set_active" | string; admin_id: string; target_user_id: string; old_value: string; new_value: string; created_at: string; admin_name: string; target_name: string };
...
export async function listAdminAuditLogs() {
  const rows = await readJson<Array<{ id: string; action: AdminAuditLog["action"]; admin_id: string; target_user_id: string; old_value: unknown; new_value: unknown; created_at: string }>>("admin_audit_log?select=id,action,admin_id,target_user_id,old_value,new_value,created_at&order=created_at.desc&limit=20");
  const ids = [...new Set(rows.flatMap((row) => [row.admin_id, row.target_user_id]))];
  ...
  return rows.map((row) => ({ ...row, old_value: valueLabel(row.old_value), new_value: valueLabel(row.new_value), admin_name: names.get(row.admin_id) || "Administrator", target_name: names.get(row.target_user_id) || "Agent" }));
}
