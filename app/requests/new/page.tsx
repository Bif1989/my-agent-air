...
export default function NewRequestPage() {
  ...
  async function handleCreate(payload: Parameters<typeof createRequest>[0]) {
    if (!session) throw new Error("Session topilmadi. Qaytadan kiring.");
    try {
      const created = await createRequest(payload, session.user.id);
      router.push(`/requests/${created.id}`);
    } catch {
      throw new Error("So‘rovni yaratib bo‘lmadi. Ma’lumotlarni tekshirib, qayta urinib ko‘ring.");
    }
  }

  return (
    <AppShell session={session} activePath="/requests">
      <div className="mx-auto max-w-4xl"><Link href="/requests" className="text-sm font-semibold text-blue-600 hover:text-blue-700 focus:outline-none focus:ring-2 focus:ring-blue-500">← So‘rovlarga qaytish</Link><header className="mt-7"><p className="text-sm font-semibold uppercase tracking-[0.18em] text-blue-600">Yangi imkoniyat</p><h1 className="mt-3 text-3xl font-semibold tracking-tight text-[#0b1f3a]">Yangi so‘rov yaratish</h1><p className="mt-2 text-sm text-slate-500">Hamkor agentlar sizga aniq taklif yuborishi uchun ma’lumotlarni kiriting.</p></header><section className="mt-8 rounded-3xl border border-slate-200 bg-white p-5 shadow-sm sm:p-8"><RequestForm submitLabel="So‘rovni yaratish" submittingLabel="Yaratilmoqda..." onSubmit={handleCreate} /></section></div>
    </AppShell>
  );
}
