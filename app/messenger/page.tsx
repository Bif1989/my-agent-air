...
function PublicCard({ room }: { room: MessengerRoom }) {
  return (
    <Link href={`/messenger/${room.id}`} className="flex items-center gap-3 rounded-2xl border border-cyan-200 bg-cyan-50/70 p-4 transition hover:border-cyan-400 hover:bg-cyan-50 focus:outline-none focus:ring-4 focus:ring-cyan-100">
      ...
      <span className="min-w-0 flex-1"><span className="block font-semibold text-[#0b1f3a]">Umumiy chat</span><span className="mt-1 block truncate text-xs text-slate-500">Barcha faol My Agent Air agentlari uchun</span></span>
...
export default function MessengerPage() {
  ...
  async function refresh() {
    setError("");
    try {
      const [room, directConversations] = await Promise.all([getPublicMessengerRoom(), listMessengerConversations()]);
      setPublicRoom(room);
      setConversations(directConversations.filter((conversation) => conversation.room_type === "direct"));
  ...
  useEffect(() => {
    ...
    const scheduleRefresh = () => {
      ...
      refreshTimer = window.setTimeout(() => { refreshTimer = null; refresh().catch(() => undefined); }, 400);
    };
  ...
  const filteredConversations = useMemo(() => {
    ...
    return conversations.filter((conversation) => [conversation.counterpart_full_name, conversation.counterpart_company_name, conversation.counterpart_city].some((value) => value?.toLocaleLowerCase("uz").includes(query)));
  }, [conversations, search]);

  return (
    <AppShell session={session} activePath="/messenger">
      <div className="mx-auto max-w-6xl">
        <header className="flex flex-col justify-between gap-4 sm:flex-row sm:items-end"><div><p className="text-sm font-semibold uppercase tracking-[0.18em] text-cyan-700">Aloqa markazi</p><h1 className="mt-3 text-3xl font-semibold tracking-tight text-[#0b1f3a] sm:text-4xl">Messenger</h1><p className="mt-2 max-w-2xl text-sm leading-6 text-slate-500">Agentlar bilan tezkor, professional va shaxsiy yozishmalar.</p></div><span className="w-fit rounded-full bg-[#0b1f3a] px-3 py-1.5 text-xs font-semibold text-cyan-200">Online aloqa</span></header>
        <div className="mt-8 grid gap-6 lg:grid-cols-[minmax(280px,390px)_1fr]">
          <section className="rounded-3xl border border-slate-200 bg-white p-4 shadow-sm sm:p-5">
            <div className="flex items-center justify-between gap-3"><h2 className="font-semibold text-[#0b1f3a]">Suhbatlar</h2><span className="text-xs text-slate-400">{conversations.length} ta shaxsiy</span></div>
            <label className="mt-4 block"><span className="sr-only">Suhbatlarni qidirish</span><input value={search} onChange={(event) => setSearch(event.target.value)} placeholder="Suhbatlarni qidiring" className="w-full rounded-xl border border-slate-200 bg-slate-50 px-4 py-3 text-sm outline-none transition placeholder:text-slate-400 focus:border-blue-400 focus:bg-white focus:ring-4 focus:ring-blue-100" /></label>
            {isLoading && <div className="mt-5 space-y-3" aria-label="Suhbatlar yuklanmoqda"><div className="h-20 animate-pulse rounded-2xl bg-slate-100" /><div className="h-20 animate-pulse rounded-2xl bg-slate-100" /></div>}
            {error && <div role="alert" className="mt-5 rounded-xl border border-red-200 bg-red-50 px-4 py-3 text-sm text-red-700">{error}</div>}
            {!isLoading && !error && <div className="mt-5 space-y-3">
              {publicRoom && <PublicCard room={publicRoom} />}
              <div className="pt-2"><p className="mb-3 px-1 text-xs font-semibold uppercase tracking-wide text-slate-400">Shaxsiy chatlar</p>{filteredConversations.length ? <div className="space-y-3">{filteredConversations.map((conversation) => <DirectCard key={conversation.room_id} conversation={conversation} />)}</div> : <p className="rounded-2xl border border-dashed border-slate-300 px-4 py-8 text-center text-sm text-slate-500">Hozircha shaxsiy yozishmalar yo‘q.</p>}</div>
            </div>}
          </section>
          <section className="hidden min-h-[520px] flex-col items-center justify-center rounded-3xl border border-slate-200 bg-[#0b1f3a] p-8 text-center text-white lg:flex"><span className="flex h-16 w-16 items-center justify-center rounded-3xl bg-cyan-300 text-3xl text-[#0b1f3a]" aria-hidden="true">✦</span><h2 className="mt-6 text-2xl font-semibold">Chatni tanlang</h2><p className="mt-2 max-w-sm text-sm leading-6 text-blue-100">Umumiy chatga qo‘shiling yoki agentlar bilan shaxsiy suhbatni boshlang.</p>{publicRoom && <Link href={`/messenger/${publicRoom.id}`} className="mt-6 rounded-xl bg-cyan-300 px-4 py-3 text-sm font-semibold text-[#0b1f3a] transition hover:bg-cyan-200 focus:outline-none focus:ring-4 focus:ring-cyan-200/40">Umumiy chatga kirish</Link>}</section>
        </div>
      </div>
    </AppShell>
  );
