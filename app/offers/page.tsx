...
function OfferCard({ offer, canManage, onEdit, onWithdraw }: { offer: OfferRecord; canManage: boolean; onEdit: (offer: OfferRecord) => void; onWithdraw: (offer: OfferRecord) => void }) {
  ...
  return (
    <article className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm sm:p-6">
      <div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
        <div>
          <Link href={`/requests/${offer.request_id}`} className="text-lg font-semibold text-[#0b1f3a] hover:text-blue-700 focus:outline-none focus:ring-2 focus:ring-blue-500">{offer.request?.origin || "—"} <span className="px-1 text-blue-500">→</span> {offer.request?.destination || "—"}</Link>
          <p className="mt-2 text-sm text-slate-500">{offer.request?.category || "Kategoriya ko‘rsatilmagan"} · {formatDate(offer.request?.travel_date || null)}</p>
        </div>
        <span className={`w-fit rounded-full px-3 py-1 text-xs font-semibold ${statusClass}`}>{statusLabels[offer.status]}</span>
      </div>
      <div className="mt-5 grid gap-3 border-t border-slate-100 pt-4 sm:grid-cols-2">
        <p className="text-xl font-semibold text-[#0b1f3a]">{offer.price == null ? "Narx ko‘rsatilmagan" : `${offer.price.toLocaleString("uz-UZ")} ${offer.currency}`}</p>
        <p className="text-sm text-slate-500">Aviakompaniya: {offer.airline || "Ko‘rsatilmagan"}</p>
        <p className="text-sm text-slate-500">Bagaj: {offer.baggage || "Ko‘rsatilmagan"}</p>
        {offer.comment && <p className="text-sm text-slate-500 sm:col-span-2">{offer.comment}</p>}
      </div>
      <div className="mt-5 flex flex-wrap items-center gap-x-4 gap-y-1 border-t border-slate-100 pt-4 text-xs text-slate-400">
        <span>{offer.agent?.full_name || "Agent nomi ko‘rsatilmagan"}</span>
        <span>{offer.agent?.company_name || "Kompaniya ko‘rsatilmagan"}</span>
        <span>{offer.agent?.city || "Shahar ko‘rsatilmagan"}</span>
        {offer.agent?.is_verified && <span className="font-semibold text-blue-600">Tasdiqlangan agent</span>}
        <span className="sm:ml-auto">{formatDate(offer.created_at)}</span>
      </div>
      {canManage && offer.status === "pending" && <div className="mt-5 flex flex-wrap gap-3">
        <button type="button" onClick={() => onEdit(offer)} className="rounded-xl border border-slate-200 px-4 py-2.5 text-sm font-semibold text-slate-700 hover:border-blue-300 hover:text-blue-700 focus:outline-none focus:ring-2 focus:ring-blue-500">Tahrirlash</button>
        <button type="button" onClick={() => onWithdraw(offer)} className="rounded-xl border border-amber-200 px-4 py-2.5 text-sm font-semibold text-amber-700 hover:bg-amber-50 focus:outline-none focus:ring-2 focus:ring-amber-500">Taklifni qaytarib olish</button>
      </div>}
    </article>
  );
}
...
export default function OffersPage() {
  ...
  useEffect(() => {
    ...
    const timeoutId = window.setTimeout(() => {
      ...
      Promise.all([getRequest(requestId), getMyOfferForRequest(requestId)])
        .then(([request, offer]) => {
          setTargetRequest(request); setTargetOffer(offer);
  ...
  async function submitEdit(payload: OfferPayload) {
    if (!editingOffer || tab !== "mine" || editingOffer.agent_id !== session?.user.id || editingOffer.status !== "pending") return;
  ...
  async function handleWithdraw(offer: OfferRecord) {
    if (tab !== "mine" || offer.agent_id !== session?.user.id || offer.status !== "pending" || !window.confirm("Taklifni qaytarib olishga ishonchingiz komilmi?")) return;
    try {
